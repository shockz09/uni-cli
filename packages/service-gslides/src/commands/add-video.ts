...
export const addVideoCommand: Command = {
  ...
  options: [
    { name: 'source', alias: 's', description: 'Video source: youtube or drive (default: youtube)', type: 'string' },
    ...
    { name: 'width', alias: 'w', description: 'Width in points (default: 400)', type: 'number' },
    { name: 'height', alias: 'h', description: 'Height in points (default: 225)', type: 'number' },
  ],
  examples: [
  ...
  async handler(ctx: CommandContext): Promise<void> {
    ...
    const source = ((options.source as string) || 'youtube').toUpperCase() as 'YOUTUBE' | 'DRIVE';
